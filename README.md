...
| `PUT`  | `/test-rw/uploads`      | Creates or appends content to the simulated upload     |
...
The first `PUT /test-rw/uploads` creates it. A missing or null content value
writes the hostname and timestamp default, while an empty list creates an empty
file. `GET /test-rw/uploads` returns HTTP 404 until that first write.
...
`uploaded.txt` on the first write.
